...
async function main() {
    ...
    let senderPvtKey = pvtKeyfromSeed("cow");
    ...
    senderPvtKey = pvtKeyfromSeed("acc1");
    to = "0x0000000000000000000000000000000001000011";
    val = 666;
    data = installCodeCallData;
    ...
    // returns Transaction Response for a simple send TX from RSK Cow account
    ...
    function pvtKeyfromSeed(seed: string){
        let seedEncoded =  new TextEncoder().encode("acc1");
    ...
    async function simpleLegacyTx(rskPvtKey: string, to: string, value: number, data?: string ) {
        ...
        console.log("The rskUser is:" +  rskUser.address +  " with balance: " + (await rskUser.getBalance()));
    ...
    // Populate a leagacy transaction
